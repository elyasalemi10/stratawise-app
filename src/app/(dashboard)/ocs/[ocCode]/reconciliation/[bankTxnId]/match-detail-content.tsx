...
export function MatchDetailContent({
  ...
  const proposalDismissalKey = pendingProposal
    ? `${ocId}:${pendingProposal.canonical_sender_name}:${pendingProposal.lot_id}`
    : "";
  ...
  }, [pendingProposal, proposalDismissed, ocId, dismissProposal]);
  ...
  const base = `/ocs/${ocCode}/reconciliation`;

  const handleOpenVoidDialog = async () => {
    setIsLoadingVoidPreview(true);
    try {
      const preview = await previewVoidBankTransaction(ocId, transaction.id);
