...
interface Props {
  params: Promise<{ ocCode: string; reportId: string }>;
}

export default async function GapReportPage({ params }: Props) {
  const { ocCode, reportId } = await params;
  ...
  const data = await getGapReportPageData(reportId, ocId);
  if (!data) notFound();
      ...
      <div className="rounded-md border border-border bg-muted/30 p-4">
        <p className="text-sm text-foreground">
          ...
          {suppressionUntil
            ...
            : "Arrears notifications are active (no suppression on this oc)."}
        </p>
      </div>
    </div>
  );
...
function TransactionRow({
  ...
  void ocId;
  const href = `/ocs/${ocCode}/reconciliation/${tx.id}`;
  return (
