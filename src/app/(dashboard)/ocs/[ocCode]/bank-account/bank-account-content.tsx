...
export function BankAccountContent({
  ...
  async function handleSave() {
    ...
    const results = await Promise.all([
      updateOCField(ocId, "bank_bsb", bsb || null),
      updateOCField(ocId, "bank_account_number", accountNumber || null),
      updateOCField(ocId, "bank_account_name", accountName || null),
    ]);
    setSaving(false);
...
function BankAccountCard({
    ...
    startTransition(async () => {
      try {
        const [txs, entries] = await Promise.all([
          getBankTransactions(ocId, account.id),
          getUndepositedEntries(ocId, account.id),
        ]);
        setTransactions(txs);
  ...
  }, [ocId, account.id, account.transaction_count, account.last_transaction_date, refreshToken]);
