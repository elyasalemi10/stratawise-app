...
function EditableInfoRow({
  ...
  const save = useCallback(async () => {
    ...
    const result = await updateLotField(ocId, lotId, field, editValue || null);
    if (result.error) {
  ...
  }, [editValue, value, ocId, lotId, field, onSaved]);
...
export function LotDetailContent({ lot: initialLot, owner, ocId, balance, documents, ownershipHistory }: LotDetailContentProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Back link */}
      <Link
        href={`/ocs/${ocCode}/manage?tab=lots`}
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <Card>
            <CardContent className="pt-5">
              ...
              <EditableInfoRow label="Lot number" value={String(lot.lot_number)} field="lot_number" lotId={lot.id} ocId={ocId} isEditing={isEditing} onSaved={(v) => onFieldSaved("lot_number", v)} />
              <EditableInfoRow label="Unit number" value={lot.unit_number} field="unit_number" lotId={lot.id} ocId={ocId} isEditing={isEditing} onSaved={(v) => onFieldSaved("unit_number", v)} />
              <EditableInfoRow label="Entitlement" value={lot.lot_entitlement ? String(lot.lot_entitlement) : null} field="lot_entitlement" lotId={lot.id} ocId={ocId} isEditing={isEditing} onSaved={(v) => onFieldSaved("lot_entitlement", v)} />
              <EditableInfoRow label="Liability" value={lot.lot_liability ? String(lot.lot_liability) : null} field="lot_liability" lotId={lot.id} ocId={ocId} isEditing={isEditing} onSaved={(v) => onFieldSaved("lot_liability", v)} />
            </CardContent>
          </Card>
