...
import { step3Schema, type Step3Values } from "@/lib/validations/oc-wizard";
import { updateOCStep3 } from "../actions";
...
export function Step3Banking({
  ...
  const {
    ...
    defaultValues: {
      bank_connection_type: initialData?.oc?.bank_connection_type ?? "manual",
  ...
  async function onSubmit(data: Step3Values) {
    ...
    setPending(true);
    const result = await updateOCStep3(ocId, { ...data, bank_name: bank });
    setPending(false);
