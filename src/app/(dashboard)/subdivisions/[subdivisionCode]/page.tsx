...
// ─── Lot Owner Dashboard ─────────────────────────────────
async function LotOwnerDashboard({ subdivisionId, subdivisionCode, profileId }: { subdivisionId: string; subdivisionCode: string; profileId: string }) {
          ...
          <Card>
            <CardContent className="pt-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href={`/subdivisions/${subdivisionCode}/my-levies`} className="text-xs text-primary hover:text-primary/80">
                  View all
                </Link>
              </div>
                    ...
                    {allLevies.map((levy) => (
                      <tr key={levy.id} className="border-t border-border/50">
                        <td colSpan={3} className="p-0">
                          <Link href={`/subdivisions/${subdivisionCode}/my-levies`} className="flex w-full hover:bg-muted/30 transition-colors">
...
export default async function SubdivisionDashboardPage({
  ...
  if (profile?.role === "lot_owner") {
    return <LotOwnerDashboard subdivisionId={subdivisionId} subdivisionCode={subdivisionCode} profileId={profile.id} />;
  }
