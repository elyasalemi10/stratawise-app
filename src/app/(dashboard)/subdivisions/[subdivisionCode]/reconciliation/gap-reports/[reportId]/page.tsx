...
interface Props {
  params: Promise<{ subdivisionCode: string; reportId: string }>;
}

export default async function GapReportPage({ params }: Props) {
  const { subdivisionCode, reportId } = await params;
...
function TransactionRow({
  ...
  void subdivisionId;
  const href = `/subdivisions/${subdivisionCode}/reconciliation/${tx.id}`;
  return (
