  ...
  return (
    <div className="space-y-5">
      ...
      <InstitutionPicker
        ...
        nominatedRepHint={
          pendingConnection?.institutionName ?? null // unused; placeholder for future
        }
      />
    </div>
  );
...
function InstitutionPicker({
  ...
  subdivisionId: string;
  nominatedRepHint?: string | null;
}) {
  const [institutions, setInstitutions] = useState<
    BasiqInstitution[] | null
  >(null);
  const [filter, setFilter] = useState("");
  const [nominatedRep, setNominatedRep] = useState("");
  ...
  async function onPick(inst: BasiqInstitution) {
    ...
    const res = await startBasiqConsent({
      ...
      return_to: `/subdivisions/new?step=4&id=${subdivisionId}`,
    });
