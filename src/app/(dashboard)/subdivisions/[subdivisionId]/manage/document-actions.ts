...
export async function getSubdivisionDocuments(subdivisionId: string): Promise<DocumentRecord[]> {
...
export async function getLotDocuments(subdivisionId: string, lotId: string): Promise<DocumentRecord[]> {
  await requireSubdivisionAccess(subdivisionId);
  const supabase = createServerClient();
...
export async function renameDocument(documentId: string, newName: string) {
  ...
  const { data: doc } = await supabase
    .from("documents")
    .select("subdivision_id, file_name")
    .eq("id", documentId)
    .single();
...
export async function deleteDocument(documentId: string) {
  ...
  const { data: doc } = await supabase
    .from("documents")
    .select("subdivision_id, file_name, file_path")
    .eq("id", documentId)
    .single();
