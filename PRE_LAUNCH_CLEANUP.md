...
- **Sequence-name drift caught during Prompt 1 verification:** base schema
  ...
  **Lesson for future consolidations: verify function bodies against the
  ...
  updated `next_reference_number()` to prepend `StrataWise-` so output matches
...
- **Operational-sequence reference format overflow:** `next_reference_number` for operational prefixes (MTG, MIN, SLEV, INV, POL, CLM, MNT, CMP, ESC) uses `lpad(seq_val::TEXT, 6, '0')`. `lpad` only pads to the minimum width; it does not truncate past 999,999. At scale a seven-digit sequence value would silently produce `StrataWise-PREFIX-YYYY-NNNNNNN` (seven digits), breaking any regex that assumes exactly six digits. Before launch: decide whether to widen the pad to 7, pre-validate against the six-digit Zod regex in `src/lib/validations/`, or add a guard that raises when the sequence crosses 999,999 on a given prefix. Unlikely in practice for single-tenant StrataWise deployments but worth naming.
...
- **`MULTI_LINKED` guard architectural assumption (PP5-B).**
  ...
  any normal StrataWise flow but allowed by the
